            ...
            <div className="section-card p-3 mb-1">
                ...
                <div className="flex flex-wrap gap-2">
                    {[
                        ...
                        { label: 'Client', value: 'Client', color: 'sky' },
            ...
            >
                {selectedUser && (
                    <div className="space-y-8">
                        {/* Header Section */}
                        <div className="flex flex-col md:flex-row items-center gap-8 p-8 rounded-[2rem] bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-slate-700/50 relative overflow-hidden group">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/5 rounded-bl-full transition-transform group-hover:scale-150 duration-700" />

                            <div className="relative">
                                <div className="w-28 h-28 rounded-[2rem] bg-white dark:bg-slate-900 border-4 border-white dark:border-slate-800 shadow-2xl flex items-center justify-center text-4xl font-black text-indigo-600 dark:text-indigo-400 overflow-hidden group-hover:scale-105 transition-transform">
                                    {selectedUser.avatar ? (
                                        <img
                                ...
                                <div className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-xl border-4 border-white dark:border-slate-900 shadow-lg flex items-center justify-center ${selectedUser.isActive ? 'bg-emerald-500 shadow-emerald-500/20' : 'bg-slate-400 shadow-slate-500/20'}`}>
                                    {selectedUser.isActive ? <BadgeCheck className="w-4 h-4 text-white" /> : <Clock className="w-4 h-4 text-white" />}
                                </div>
                            </div>

                            <div className="text-center md:text-left space-y-2 relative z-10">
                                <div className="flex flex-col md:flex-row items-center gap-3">
                                    <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase leading-none">{selectedUser.name}</h2>
                                    <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border-2 ${selectedUser.role === 'Admin' ? 'bg-indigo-500 text-white border-indigo-500' :
                                        selectedUser.role === 'Manager' ? 'bg-emerald-500 text-white border-emerald-500' :
                                            'bg-slate-200 text-slate-700 border-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:border-slate-700'
                                        }`}>
                                        {selectedUser.role}
                                    </span>
                                </div>
                                <div className="flex items-center justify-center md:justify-start gap-4 text-slate-500 dark:text-slate-400">
                                    <div className="flex items-center gap-1.5 text-sm font-bold uppercase tracking-widest">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {['Admin', 'Manager', 'Employee', 'Client'].map((role) => (
                                    <label
                                        key={role}
